...
public class HorizontalExpandMenu extends RelativeLayout {
    ...
    private void init() {
        ...
        defaultWidth = ScreenUtils.dip2px(mContext, 200);
        defaultHeight = ScreenUtils.dip2px(mContext, 40);
        ...
        menuCornerRadius = typedArray.getDimension(R.styleable.HorizontalExpandMenu_corner_radius, ScreenUtils.dip2px(mContext, 20));

        buttonStyle = typedArray.getInteger(R.styleable.HorizontalExpandMenu_button_style, ButtonStyle.Right);
        buttonIconDegrees = 0;
        buttonIconSize = typedArray.getDimension(R.styleable.HorizontalExpandMenu_button_icon_size, ScreenUtils.dip2px(mContext, 8));
     ...
     * @param time 动画时间
     */
    private void expandMenu(final int time) {
        ...
        post(new Runnable() {
            @Override
            public void run() {
                if (mOnExpandMenuListener!=null){
                    mOnExpandMenuListener.onExpand(isExpand,time);
                }
            }
        });
    }
    ...
    private OnExpandMenuListener mOnExpandMenuListener;

    public void setOnExpandMenuListener(OnExpandMenuListener onExpandMenuListener) {
        this.mOnExpandMenuListener = onExpandMenuListener;
    }

    public interface OnExpandMenuListener{

        void onExpand(boolean isExpand,int time);
